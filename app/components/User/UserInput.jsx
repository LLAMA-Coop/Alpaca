...
export function UserInput({ isRegistering, onSubmit }) {
  ...
  async function handleTwoFactor(e) {
    ...
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_BASEPATH ?? ""}/api/auth/2fa`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
  ...
  return (
    <Form onSubmit={isRegistering ? handleRegister : handleLogin}>
      ...
      {isRegistering && (
        <InfoBox asDiv fullWidth className={styles.hideBox}>
      ...
      <Input
        type="checkbox"
        label={revealPass ? "Hide Password" : "Show Password"}
        value={revealPass}
        onChange={(e) => {
      ...
      <Link href="/forgot-password" className={`link ${styles.forgotPassword}`}>
        Forgot password?
      </Link>
