...
export const validation = {
    ...
    report: {
        type: {
            ...
            error: "Must be one of 'spam', 'harassment', 'hate-speech', 'violence', 'nudity', or 'other'",
        },
...
export class Validator {
    ...
    validate({ field, value, type, isSubfield }) {
        ...
        const item = !isSubfield ? validation[type][field] : validation[type][field]["values"];

        const { type: t, maxSize, maxLength, minLength, regex, error, values } = item;
    ...
    validatePermissions(
        ...
        if (typeof allRead !== "boolean" && ![0, 1].includes(allRead)) {
            errors.allRead = "allRead must be a boolean";
        }

        if (typeof allWrite !== "boolean" && ![0, 1].includes(allWrite)) {
            errors.allWrite = "allWrite must be a boolean";
        }

        if (typeof groupLocked !== "boolean" && ![0, 1].includes(groupLocked)) {
        ...
        [...read, ...write].forEach((p) => {
            if (typeof p !== "number") {
                errors.read = "read and write must be arrays of numbers";
                errors.write = "read and write must be arrays of numbers";
            }
        });
