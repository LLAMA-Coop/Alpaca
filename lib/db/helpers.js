...
export async function getPermittedResources({
      ...
      .select(({ eb }) => {
        ...
        if (type !== "source") {
          ...
          addedFields.push({
            ...
            asJoin: "s",
            condition: `rr.A = ${as}.id AND rr.A_type = "${type}" AND rr.B_type = "source"`,
            multiple: true,
          });
        }
        ...
        if (type !== "note") {
          ...
          addedFields.push({
            ...
            join: `INNER JOIN notes as n ON rr.B = n.id`,
            asJoin: "n",
            condition: `rr.A = ${as}.id AND rr.A_type = "${type}" AND rr.B_type = "note"`,
            multiple: true,
          });
        }
        ...
        if (type === "quiz") {
          ...
          // Get hiddenUntil from 'user_quizzes'
          fields.push({
            field: "hiddenUntil",
            query: `IFNULL((SELECT hidden_until FROM user_quizzes WHERE quiz_id = ${as}.id AND user_id = ${
              userId || null
            }), 0)`,
          });
  ...
  try {
    ...
    const resources = await query.executeTakeFirstOrThrow();
...
export async function canDeleteResource(userId, resourceId, resourceTable) {
  try {
    const resource = await db
      .selectFrom(resourceTable)
      .select("id")
      .where("id", "=", resourceId)
      .where("createdBy", "=", userId)
      .executeTakeFirst();
