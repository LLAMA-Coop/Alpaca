...
export async function addError(error, funcName) {
    const { name, message, code, stack, sql } = error;

    const [errorInsert, fields] = await db
        .promise()
        .query(
            "INSERT INTO `ErrorsBugs` (`function`, `name`, `message`, `code`, `stack`, `sql`) VALUES (?, ?, ?, ?, ?, ?)",
            [
                funcName ? funcName.substring(0, 128) : undefined,
                name ? name.substring(0, 128) : undefined,
                message ? message.substring(0, 1024) : undefined,
                code ? code.substring(0, 128) : undefined,
                stack ? stack.substring(0, 1024) : undefined,
                sql ? sql.substring(0, 1024) : undefined,
            ],
        );
...
export async function updateSource(source) {
    ...
    try {
        ...
        if (courses && courses.length > 0) {
            courses.forEach((crs) => {
                const courseId = typeof crs === "object" ? crs.id : crs;
                const includeReferencingResources =
                    typeof crs === "object"
...
export async function upsertTable(tableName, data, updateColumns) {
    try {
        ...
        const query = `INSERT IGNORE INTO ${tableName} (${columnNames}) VALUES (${questionMarks}) ON DUPLICATE KEY UPDATE ${updateClause}`;
...
export async function insertCourseResources({
    courseIDs,
    resourceId,
    resourceType,
    includeReferencingResources,
    ...
    try {
        const baseQuery =
            "INSERT INTO `CourseResources` (`courseId`, `resourceId`, `resourceType`, `includeReferencingResources`) VALUES ?";

        const fieldsArray = [];
        courseIDs.forEach((crs) => {
            fieldsArray.push([
                crs,
                resourceId,
                resourceType,
        ...
        console.error("Error in insertCourseResources", error);
        addError(error, "insertCourseResources");
        throw error;
    }
}
...
export async function insertPermissions(permissions, resourceId, permittedId) {
    if (!permissions) {
        throw Error(
            "A permissions array is required in order to use insertPermissions",
        );
    }
