...
export async function getRelationships({ type, id }) {
  const resourceList = ["source", "note", "quiz", "course", "group"].filter(
    (x) => x !== type
  );
  const rtnResource = {};
  ...
  const relationships = await db
    ...
    .where((eb) =>
      eb.or([
        eb.and([eb("A_type", "=", type), eb("A", "=", id)]),
        eb.and([eb("B_type", "=", type), eb("B", "=", id)]),
      ])
    )
    .execute();
