...
// Redo the resource permission structure
...
export async function getPermittedResources({
  ...
  function filterToPermitted({ eb, table, as, type, getHasMore }) {
    return eb
      .selectFrom(table)
      .select("id")
      .innerJoin("resource_permissions as rp", (join) =>
        join
          .onRef(`${table}.id`, "=", "rp.resourceId")
          .on("rp.resourceType", "=", type),
      )
      ...
      .$if(groupId, (eb) =>
        eb.where(({ eb, and, or }) =>
          or([
            ...
            and([
              sql`JSON_CONTAINS(rp.read, CAST(${groupId} AS JSON))`,
              sql`JSON_CONTAINS(rp.write, CAST(${groupId} AS JSON))`,
              eb("rp.groupLocked", "=", 0),
            ]),
          ]),
        ),
      ...
      .$if(userId, (eb) =>
        eb.where(({ eb, exists, or, and }) =>
          or([
            ...
            and([
              or([
                sql`JSON_CONTAINS(rp.read, CAST(${userId} AS JSON))`,
                sql`JSON_CONTAINS(rp.write, CAST(${userId} AS JSON))`,
                eb("rp.allRead", "=", 1),
                eb("rp.allWrite", "=", 1),
              ]),
              or([
  ...
  try {
    await db
      ...
      .onDuplicateKeyUpdate({
        ...
        timeSpent: sql`time_spent + ${timeSpent}`,
        lastAccessed: new Date(),
        updatedAt: new Date(),
      })
      .execute();
    ...
    catchRouteError({ error, route: "trackUserProgress" });
    return false;
  }
}
  ...
  try {
    const result = await db
      .selectNoFrom((eb) => [
        ...
        eb
          .selectFrom("user_progress")
          .select((eb) => eb.fn.count("id").as("completed"))
          .where("userId", "=", userId)
          .where("courseId", "=", courseId)
          .where("isCompleted", "=", 1)
          .as("completedCount"),
    ...
    catchRouteError({ error, route: "getUserCourseProgress" });
    return { completedCount: 0, totalCount: 0, percentage: 0 };
  }
}
...
export async function getUserStreak(userId) {
  ...
  try {
    ...
    catchRouteError({ error, route: "getUserStreak" });
    return { currentStreak: 0, longestStreak: 0 };
  }
}
...
export async function updateUserStreak(userId) {
  ...
  try {
    ...
    const streak = await db
      .selectFrom("daily_streaks")
      .select(["currentStreak", "lastActivityDate"])
      .where("userId", "=", userId)
      .executeTakeFirst();
    ...
    catchRouteError({ error, route: "updateUserStreak" });
    return false;
  }
}
...
export async function getRecentActivity(userId, limit = 10) {
  ...
  try {
    ...
    catchRouteError({ error, route: "getRecentActivity" });
    return [];
  }
}

const XP_WEIGHTS = { quiz: 10, note: 5, source: 2 };

export async function getUserXP(userId) {
  if (!isValidId(userId)) {
    return { xp: 0, level: 1, xpIntoLevel: 0, xpForNextLevel: 100 };
  }

  try {
    const rows = await db
      .selectFrom("user_progress")
      .select(["resourceType", db.fn.count("id").as("count")])
      .where("userId", "=", userId)
      .where("isCompleted", "=", 1)
      .groupBy("resourceType")
      .execute();

    const xp = rows.reduce((sum, row) => {
      const weight = XP_WEIGHTS[row.resourceType] || 1;
      return sum + weight * Number(row.count);
    }, 0);
    ...
    catchRouteError({ error, route: "getUserXP" });
    return { xp: 0, level: 1, xpIntoLevel: 0, xpForNextLevel: 100 };
  }
}

export async function getTodayStudySeconds(userId) {
  if (!isValidId(userId)) {
    return 0;
  }
  ...
  try {
    ...
    const row = await db
      .selectFrom("user_progress")
      .select(db.fn.sum("timeSpent").as("total"))
      .where("userId", "=", userId)
      .where("lastAccessed", ">=", startOfDay)
      .executeTakeFirst();

    return Number(row?.total) || 0;
  } catch (error) {
    catchRouteError({ error, route: "getTodayStudySeconds" });
    return 0;
  }
}
...
export async function getLeaderboard({ limit = 5 } = {}) {
  try {
    const xpExpr = sql`SUM(CASE up.resource_type WHEN 'quiz' THEN 10 WHEN 'note' THEN 5 ELSE 2 END)`;

    const rows = await db
      .selectFrom("user_progress as up")
      ...
      .groupBy(["up.userId", "u.username", "u.displayName", "u.avatar"])
      .orderBy("xp", "desc")
      .limit(limit)
      .execute();
    ...
    catchRouteError({ error, route: "getLeaderboard" });
    return [];
  }
}

export async function getStreakCalendar(userId, days = 7) {
  if (!isValidId(userId)) {
    return [];
  }
  ...
  try {
    ...
    const rows = await db
      .selectFrom("user_progress")
      .select([sql`DATE(COALESCE(completed_at, last_accessed))`.as("day")])
      .where("userId", "=", userId)
      .where(({ eb, or }) =>
        or([eb("completedAt", ">=", since), eb("lastAccessed", ">=", since)])
      )
      .groupBy("day")
      .execute();

    const activeDays = new Set(
      rows.filter((row) => row.day).map((row) => new Date(row.day).toDateString())
    );
    ...
    for (let i = days - 1; i >= 0; i--) {
      ...
      calendar.push({
        ...
        completed: activeDays.has(date.toDateString()),
    ...
    catchRouteError({ error, route: "getStreakCalendar" });
    return [];
  }
}
