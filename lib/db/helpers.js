...
if (!accessTokenSecret || !refreshTokenSecret) {
    console.error("Please provide ACCESS_TOKEN_SECRET and REFRESH_TOKEN_SECRET in the .env file.");
    process.exit(1);
}
...
export const noteDefaultColumns = ["id", "title", "text", "tags", "createdAt"];
...
export const courseDefaultColumns = ["id", "name", "description", "enrollment", "createdAt"];

export const userDefaultColumns = ["id", "username", "displayName", "avatar", "createdAt"];
...
export function jsonObject({ list, table, subFields, eb }) {
    ...
    if (subFields?.length) {
        return sql`JSON_OBJECT(
            ...
            ${sql.raw(
                subFields
                    .map(({ field, table, as, columns, condition, multiple, join, asJoin }) => {
                        if (multiple && join) {
                            return `, '${field}', (SELECT IFNULL((SELECT JSON_ARRAYAGG(JSON_OBJECT(
                                    ${columns
                                        .map((c) => `'${c}', ${asJoin}.${camelToSnake(c)}`)
                                        .join(", ")}
...
export async function getPermittedResources({
    ...
    function filterToPermitted({ eb, table, as, type, ids, getHasMore }) {
        return eb
            .selectFrom(table)
            .select("id")
            ...
            .$if(tagSearch && type !== "course", (eb) =>
                eb.where(sql`JSON_CONTAINS(${sql.ref(`${table}.tags`)}, '"${sql.raw(tagSearch)}"')`)
            )
    ...
    function hasMore({ eb, table, as, type, ids }) {
        return (
            eb
                .selectFrom(`${table} as ${as}`)
                // Returns true if there are more resources than the limit
                .innerJoin(
                    (eb) => filterToPermitted({ eb, table, as, type, ids, getHasMore: true }),
                    (join) => join.onRef(`${as}.id`, "=", `${as}2.id`)
                )
                .select(sql`IF(COUNT(${sql.ref(`${as}.id`)}) > ${limit}, TRUE, FALSE)`)
                .as(`hasMore${firstLetterUpperCase(table)}`)
        );
    }
            ...
            .select(({ eb }) => {
                ...
                if (type !== "course") {
                    ...
                    addedFields.push({
                        ...
                        join: `INNER JOIN courses as c ON rr.B = c.id`,
                        asJoin: "c",
                        condition: `rr.A = ${as}.id AND rr.A_type = "${type}" AND rr.B_type = "course"`,
                        multiple: true,
                    });
                }
                ...
                if (type === "course") {
                    ...
                    addedFields.push({
                        ...
                        join: `INNER JOIN sources as s2 ON rr.B = s2.id`,
                        asJoin: "s2",
                        condition: `rr.A = ${as}.id AND rr.A_type = "course" AND rr.B_type = "source"`,
                        multiple: true,
                    });
                    ...
                    addedFields.push({
                        ...
                        table: "resource_relations",
                        as: "rr",
                        columns: noteDefaultColumns,
                        join: `INNER JOIN notes as n2 ON rr.B = n2.id`,
                        asJoin: "n2",
                        condition: `rr.A = ${as}.id AND rr.A_type = "course" AND rr.B_type = "note"`,
                        multiple: true,
                    });
                    ...
                    addedFields.push({
                        ...
                        table: "resource_relations",
                        as: "rr",
                        columns: quizDefaultColumns,
                        join: `INNER JOIN quizzes as q2 ON rr.B = q2.id`,
                        asJoin: "q2",
                        condition: `rr.A = ${as}.id AND rr.A_type = "course" AND rr.B_type = "quiz"`,
                        multiple: true,
                    });
                ...
                if (type === "note") {
                    ...
                    addedFields.push({
                        ...
                        join: `INNER JOIN sources as s2 ON rr.B = s2.id`,
                        asJoin: "s2",
                        condition: `rr.A = ${as}.id AND rr.A_type = "note" AND rr.B_type = "source"`,
                        multiple: true,
                    });
                }
        ...
        const resources = await db
            .selectNoFrom((eb) => {
                ...
                if (withSources || takeAll) {
                    toSelect.push(
                        helper(eb, "sources", "s", "source", sourceDefaultColumns, sourceIds)
                    );

                    // Here I want to add a hasMoreSources field to the response
                    toSelect.push(
                        hasMore({ eb, table: "sources", as: "s", type: "source", ids: sourceIds })
                    );
                }

                if (withNotes || takeAll) {
                    toSelect.push(helper(eb, "notes", "n", "note", noteDefaultColumns, noteIds));

                    // Here I want to add a hasMoreNotes field to the response
                    toSelect.push(
                        hasMore({ eb, table: "notes", as: "n", type: "note", ids: noteIds })
                    );
                }

                if (withQuizzes || takeAll) {
                    toSelect.push(helper(eb, "quizzes", "q", "quiz", quizDefaultColumns, quizIds));

                    // Here I want to add a hasMoreQuizzes field to the response
                    toSelect.push(
                        hasMore({ eb, table: "quizzes", as: "q", type: "quiz", ids: quizIds })
                    );
                }

                if (withCourses || takeAll) {
                    toSelect.push(
                        helper(eb, "courses", "c", "course", courseDefaultColumns, courseIds)
                    );

                    // Here I want to add a hasMoreCourses field to the response
                    toSelect.push(
                        hasMore({ eb, table: "courses", as: "c", type: "course", ids: courseIds })
                    );
                }
...
export async function addRelationsNoDuplicates({ idA, typeA, typeB, newRelations }) {
    try {
        await db
...
export async function updateSource({
    ...
    try {
        if (title || medium || url || publishedAt || lastAccessed || tags || authors) {
            await db
                .updateTable("sources")
                .set({
                    title,
        ...
        return catchRouteError({ error, route: "updateSource", skipApiResponse: true });
    }
}
...
export async function doesUserMeetPrerequisites(courseId) {
    ...
    try {
        const prerequisites = await db
            ...
            .select(["p.averageLevelRequired", "p.minimumLevelRequired", "c.progress"])
