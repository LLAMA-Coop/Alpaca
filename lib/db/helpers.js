...
export async function getQuizzesById({ id, ids }) {
    let baseQuery =
        "SELECT `Quizzes`.`id`, `Quizzes`.`type`, `Quizzes`.`prompt`, `Quizzes`.`choices`, `Quizzes`.`correctResponses`, `Quizzes`.`hints`, `Quizzes`.`tags`, `Creator`.`id`, `Creator`.`username`, `Creator`.`displayName`, `Creator`.`description`, `Creator`.`avatar`, `Contributors`.`id`, `Contributors`.`username`, `Contributors`.`displayName`, `Contributors`.`description`, `Contributors`.`avatar`, FROM QUIZ LEFT JOIN `Users` AS `Creator` ON `Creator`.`id` = `Quizzes`.`createdBy` LEFT JOIN `ResourceContributors` ON `Quizzes`.`id` = `ResourceContributors`.`resourceId` AND `ResourceContributors`.`resourceType` = 'quiz' LEFT JOIN `Users` AS `Contributors` ON `ResourceContributors`.`id` = `Contribors`.`id` WHERE ";

    baseQuery += ids && ids.length > 0 ? "`id` IN ?" : "`id` = ?";
