...
export async function getQuizzesById({ id, ids }) {
    let baseQuery =
        "SELECT `Quizzes`.`id`, `Quizzes`.`type`, `Quizzes`.`prompt`, `Quizzes`.`choices`, `Quizzes`.`correctResponses`, `Quizzes`.`hints`, `Quizzes`.`tags`, `Creator`.`id` AS `creatorId`, `Creator`.`username` AS `creatorName`, `Creator`.`displayName` AS `creatorDisplay`, `Creator`.`description` AS `creatorDesc`, `Creator`.`avatar` AS `creatorAvatar`, `Contributors`.`id` AS `contribId`, `Contributors`.`username` AS `contribName`, `Contributors`.`displayName` AS `contribDisplay`, `Contributors`.`description` AS `contribDesc`, `Contributors`.`avatar` AS `contribAvatar`, `CourseResources`.`id` AS `courseId` FROM QUIZZES LEFT JOIN `Users` AS `Creator` ON `Creator`.`id` = `Quizzes`.`createdBy` LEFT JOIN `ResourceContributors` ON `Quizzes`.`id` = `ResourceContributors`.`resourceId` AND `ResourceContributors`.`resourceType` = 'quiz' LEFT JOIN `Users` AS `Contributors` ON `ResourceContributors`.`userId` = `Contributors`.`id` LEFT JOIN `CourseResources` ON `Quizzes`.`id` = `CourseResources`.`resourceId` AND `CourseResources`.`resourceType` = 'quiz' WHERE ";

    baseQuery +=
        ids && ids.length > 0 ? "`Quizzes`.`id` IN (?)" : "`Quizzes`.`id` = ?";
...
//     const baseQuery =
//         "SELECT `Groups`.`id`, `Groups`.`name`, `Groups`.`publicId`, `Groups`.`description`, `Groups`.`isPublic`, `Groups`.`avatar`, `Members`.`id`, `Members`.`userId` AS `memberId`, `Members`.`role` FROM `Groups` LEFT JOIN `Members` on `Members`.`groupId` = `Groups`.`id` WHERE `Groups`.`id` = ?";
