...
export async function getPermittedResources({
    ...
    if (!withSources && !withNotes && !withQuizzes && !withCourses && !takeAll) {
    ...
    function helper(eb, table, as, type, columns) {
        return eb
            ...
            .innerJoin("resource_permissions as rp", `${as}.id`, "rp.resourceId")
            .where("rp.resourceType", "=", type)
            .$if(!userId && !groupId, (eb) =>
                eb.where(({ eb, and }) =>
                    and([eb("rp.allRead", "=", 1), eb("rp.groupLocked", "=", 0)])
                )
            )
            .$if(groupId, (eb) =>
                eb.where(({ eb, or }) =>
                    or([eb("rp.groupId", "=", groupId), sql`JSON_CONTAINS(rp.read, ${groupId})`])
                )
            )
            .$if(userId, (eb) =>
        ...
        const resources = await db
            .selectNoFrom((eb) => {
                ...
                if (withSources || takeAll) {
                    toSelect.push(helper(eb, "sources", "s", "source", sourcesColumns));
                }

                if (withNotes || takeAll) {
                    toSelect.push(helper(eb, "notes", "n", "note", notesColumns));
                }

                if (withQuizzes || takeAll) {
                    toSelect.push(helper(eb, "quizzes", "q", "quiz", quizzesColumns));
                }

                if (withCourses || takeAll) {
                    toSelect.push(helper(eb, "courses", "c", "course", coursesColumns));
                }
